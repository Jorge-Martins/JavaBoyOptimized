...
class GameBoyPrinter extends GameLink {
    ...
    * 
    */
      private static final long serialVersionUID = 6939070268427364962L;
      Image i;
      int scale = 2;
   ...
   GameBoyPrinter() {
      ...
      source = new MemoryImageSource(IMAGE_WIDTH, IMAGE_HEIGHT, new DirectColorModel(32,
               0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000), imageData, 0, IMAGE_WIDTH);
   ...
   void send(byte b) {
      System.out.print(b + " ");
      cpu.ioHandler.registers[0x01] = 0x00; // Acknowledge the byte by sending a
                                            // zero
      ...
      if ((getByte(0) == 136)) {
         ...
         if ((bytesAvailable() == 16) && (dataSize > 0)) {
            for (int offs = 0; offs < 8; offs++) {
               ...
               imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8) + 0] = palette[((b1 & 0x80) >> 6)
                        + ((b2 & 0x80) >> 7)];
               imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8) + 1] = palette[((b1 & 0x40) >> 5)
                        + ((b2 & 0x40) >> 6)];
               imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8) + 2] = palette[((b1 & 0x20) >> 4)
                        + ((b2 & 0x20) >> 5)];
               imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8) + 3] = palette[((b1 & 0x10) >> 3)
                        + ((b2 & 0x10) >> 4)];

               imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8) + 4] = palette[((b1 & 0x08) >> 2)
                        + ((b2 & 0x08) >> 3)];
               imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8) + 5] = palette[((b1 & 0x04) >> 1)
                        + ((b2 & 0x04) >> 2)];
               imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8) + 6] = palette[((b1 & 0x02) >> 0)
                        + ((b2 & 0x02) >> 1)];
               imageData[((tileY * 8 + offs) * IMAGE_WIDTH) + (tileX * 8) + 7] = palette[((b1 & 0x01) << 1)
                        + ((b2 & 0x01) >> 0)];
