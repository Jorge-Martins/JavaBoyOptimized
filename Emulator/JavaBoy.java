...
public class JavaBoy extends java.applet.Applet implements Runnable, KeyListener, WindowListener,
   ...
   /**
    * These strings contain all the names for the colour schemes. A scheme can
    * be activated using the view menu when JavaBoy is running as an
    * application.
    */
   static public String[] schemeNames = { "Standard colours", "LCD shades", "Midnight garden",
            "Psychadelic" };

   /**
    * This array contains the actual data for the colour schemes. These are only
    * using in DMG mode. The first four values control the BG palette, the
    * second four are the OBJ0 palette, and the third set of four are OBJ1.
    */
   static public int[][] schemeColours = {
            { 0xFFFFFFFF, 0xFFAAAAAA, 0xFF555555, 0xFF000000, 0xFFFFFFFF, 0xFFAAAAAA, 0xFF555555,
                     0xFF000000, 0xFFFFFFFF, 0xFFAAAAAA, 0xFF555555, 0xFF000000 },

            { 0xFFFFFFC0, 0xFFC2C41E, 0xFF949600, 0xFF656600, 0xFFFFFFC0, 0xFFC2C41E, 0xFF949600,
                     0xFF656600, 0xFFFFFFC0, 0xFFC2C41E, 0xFF949600, 0xFF656600 },

            { 0xFFC0C0FF, 0xFF4040FF, 0xFF0000FF, 0xFF000080, 0xFFC0FFC0, 0xFF00C000, 0xFF008000,
                     0xFF004000, 0xFFC0FFC0, 0xFF00C000, 0xFF008000, 0xFF004000 },

            { 0xFFFFC0FF, 0xFF8080FF, 0xFFC000C0, 0xFF800080, 0xFFFFFF40, 0xFFC0C000, 0xFFFF4040,
                     0xFF800000, 0xFF80FFFF, 0xFF00C0C0, 0xFF008080, 0xFF004000 } };
   ...
   /**
    * When emulation running, references the current graphics chip
    * implementation
    */
   GraphicsChip graphicsChip;

   /**
    * When connected to another computer or to a Game Boy printer, references
    * the current Game link object
    */
   GameLink gameLink;
   ...
   /**
    * When running as an application, contains a reference to the interface
    * frame object
    */
   ...
   public void paint(Graphics g) {
      if (dmgcpu != null) {
         ...
         if ((stripTimer > stripLength) && (!fullFrame) && (!imageSizeChanged)) {
             ...
             * (Math.random() * 255))); g.fillRect(0, 0, getSize().width,
             * getSize().height); imageSizeChanged = false; if (fullFrame) {
             * System.out.println("fullframe is " + fullFrame + " at "+
            ...
            if (dmgcpu.graphicsChip.isFrameReady()) {
               ...
               if (stripTimer < stripLength) {
                  if (stripTimer < stripLength / 2) {
                     ...
                     bufferGraphics.drawString("Emulator version: " + versionString, 2,
                              stripPos + 36);
                  }
               }
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      if (e.getActionCommand().equals("Size: 1x")) {
         ...
         try {
            dmgcpu.cartridge.saveBatteryRAMToWeb(new URL(getParameter("SAVERAMURL")),
                     getParameter("USERNAME"), dmgcpu);
         ...
         try {
            ...
            dmgcpu.cartridge.loadBatteryRAMFromWeb(new URL(getParameter("LOADRAMURL")),
                     getParameter("USERNAME"), dmgcpu);
            // do {
   ...
   /**
    * Execute any pending debugger commands, or get a command from the console
    * and execute it
    */
         ...
         switch (st.nextToken().charAt(0)) {
            ...
            case 'd':
               try {
                  ...
                  System.out.println("- Dumping " + JavaBoy.hexWord(length)
                           + " instructions starting from " + JavaBoy.hexWord(address));
            ...
            case 'i':
               try {
                  int address = Integer.valueOf(st.nextToken(), 16).intValue();
                  int length = Integer.valueOf(st.nextToken(), 16).intValue();
                  System.out.println("- Dissasembling " + JavaBoy.hexWord(length)
                           + " instructions starting from " + JavaBoy.hexWord(address));
            ...
            case 'm':
               try {
                  ...
                  System.out.println("- Mapping ROM bank " + JavaBoy.hexByte(bank)
                           + " to 4000 - 7FFFF");
                  cartridge.saveMapping();
            ...
            case 't':
               try {
                  ...
                  System.out.println("- Executing instruction at program counter ("
                           + JavaBoy.hexWord(dmgcpu.pc) + ")");
                  dmgcpu.execute(1);
               } catch (NumberFormatException e) {
                  System.out.println("Error parsing hex value.");
   ...
   public static void main(String[] args) {
      System.out.println("JavaBoy (tm) Version " + versionString
