...
class NoiseGenerator {
   ...
   /**
    * Channel being played on. Combination of CHAN_LEFT and CHAN_RIGHT, or
    * CHAN_MONO
    */
   int channel;
   ...
   /**
    * Creates a white noise generator with the specified wavelength, amplitude,
   ...
   public void setParameters(float dividingRatio, boolean polynomialSteps, int shiftClockFreq) {
      ...
      // System.out.println("dr:" + dividingRatio + "  steps: " +
      // this.polynomialSteps + "  shift:" + shiftClockFreq + "  = Freq:" +
      // finalFreq);
   }

   /**
    * Output a single frame of samples, of specified length. Start at position
   ...
   public void play(byte[] b, int length, int offset) {
      if (totalLength != 0) {
         ...
         /*
          * for (int r = offset; r < offset + length; r++) { val = (int)
          * ((Math.random() * amplitude * 2) - amplitude);
          * 
          * if ((channel & CHAN_LEFT) != 0) b[r * 2] += val; if ((channel &
          * CHAN_RIGHT) != 0) b[r * 2 + 1] += val; if ((channel & CHAN_MONO) !=
          * 0) b[r] += val;
          * 
