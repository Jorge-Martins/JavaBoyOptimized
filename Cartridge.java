...
class Cartridge {
  ...
  */
   final int[][] romSizeTable = { { 0, 2 }, { 1, 4 }, { 2, 8 }, { 3, 16 }, { 4, 32 }, { 5, 64 },
            { 6, 128 }, { 7, 256 }, { 0x52, 72 }, { 0x53, 80 }, { 0x54, 96 } };

   /**
    * Contains strings of the standard names of the cartridge mapper chips,
    * indexed by cartridge type
    */
    ...
    * the cartridge filename given. Loads via the web if JavaBoy is running as
    * an applet
    */
   public Cartridge(String romFileName, Component a) {
      ...
      try {
         ...
         int total = 0x04000;
         do {
            total -= is.read(firstBank, 0x04000 - total, total); // Read the
                                                                 // first bank
                                                                 // (bank 0)
         } while (total > 0);
         ...
         numBanks = lookUpCartSize(firstBank[0x0148]); // Determine the number
                                                       // of 16kb rom banks

         // is.close();
         // is = new FileInputStream(new File(romFileName));
         rom = new byte[0x04000 * numBanks]; // Recreate the ROM array with the
                                             // correct size
         ...
         do { // Read ROM into memory
            total -= is.read(rom, rom.length - total, total); // Read the entire
                                                              // ROM
         } while (total > 0);
         is.close();

         JavaBoy.debugLog("Loaded ROM '" + romFileName + "'.  " + numBanks + " banks, "
                  + (numBanks * 16) + "Kb.  " + getNumRAMBanks() + " RAM banks.");
         JavaBoy.debugLog("Type: " + cartTypeTable[cartType] + " (" + JavaBoy.hexByte(cartType)
                  + ")");

         if (!verifyChecksum() && (a instanceof Frame)) {
            new ModalDialog((Frame) a, "Warning", "This cartridge has an invalid checksum.",
   ...
   public InputStream openRom(String romFileName, Component a) {
      ...
      if (bFormat == bZip) {
         ...
         try {
            ...
            if (JavaBoy.runningAsApplet) {
               zip = new java.util.zip.ZipInputStream(new java.net.URL(
                        ((Applet) (a)).getDocumentBase(), romFileName).openStream());
            } else {
      ...
      if (bFormat == bGZip) {
         ...
         try {
            if (JavaBoy.runningAsApplet) {
               return new java.util.zip.GZIPInputStream(new java.net.URL(
                        ((Applet) (a)).getDocumentBase(), romFileName).openStream());
            } else {
   ...
   /**
    * Returns the byte currently mapped to a CPU address. Addr must be in the
    * range 0x0000 - 0x4000 or 0xA000 - 0xB000 (for RAM access)
    */
   ...
   /**
    * Writes a byte to an address in CPU address space. Identical to
    * addressWrite() except that writes to ROM do not cause a mapping change,
    * but actually write to the ROM. This is usefull for patching parts of code.
   ...
   public final void addressWrite(int addr, int data) {
      ...
      switch (cartType) {
         ...
         case 0x13: /* MBC3 */
             ...
             * { mbc1LargeRamMode = true; System.out.println("Small Ram"); //
             * ram = new byte[0x8000]; } else { mbc1LargeRamMode = false;
   ...
   public void outputCartInfo() {
      ...
      String infoString = "ROM Info: Name = " + cartName + ", Size = " + (numBanks * 128)
               + "Kbit, ";
...
class WebSaveRAM implements Runnable, DialogListener {
   ...
   public WebSaveRAM(URL url, boolean save, Cartridge cart, Dmgcpu cpu, String username) {
      ...
      if (!cart.canSave()) {
         ...
         ModalDialog d = new ModalDialog(null, "Sorry", "This game does not",
   ...
   public void run() {
      ...
      try {
         ...
         System.out.println("Error! " + e);
         new ModalDialog(null, "Error!", "Load/Save error!  Report to site administrator.",
                  e.toString());
      }
      f.hide();
   }
   ...
   public void saveRam() throws Exception {
      ...
      url = new URL(url.getProtocol(), url.getHost(), url.getPort(), url.getFile() + "?user="
      ...
      String content = "romname=" + URLEncoder.encode(cart.getRomFilename()) + "&gamename="
               + URLEncoder.encode(cart.getCartName()) + "&user=" + URLEncoder.encode(username)
               + "&datalength=" + (cart.getBatteryRamSize() * 2) + "&data0=" + saveData + params;
   ...
   public void loadRam() throws Exception {
      ...
      url = new URL(url.getProtocol(), url.getHost(), url.getPort(), url.getFile() + "?user="
               + URLEncoder.encode(username) + params);
